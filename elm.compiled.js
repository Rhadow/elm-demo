...
Elm.App.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var UpdateIP = function (a) {    return {ctor: "UpdateIP",_0: a};};
   var requestIP = $Effects.task(A2($Task.map,UpdateIP,$Task.toMaybe(A2($Http.get,A2($Json$Decode._op[":="],"ip",$Json$Decode.string),"http://jsonip.com"))));
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "DoNothing": return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
         case "RequestIP": return {ctor: "_Tuple2",_0: model,_1: requestIP};
         default: return {ctor: "_Tuple2",_0: _U.update(model,{ip: A2($Maybe.withDefault,"No response",_p0._0)}),_1: $Effects.none};}
   });
   var RequestIP = {ctor: "RequestIP"};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,RequestIP)]),_U.list([$Html.text("Get IP address")]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text(model.ip)]))]));
   });
   var DoNothing = {ctor: "DoNothing"};
   var init = {ctor: "_Tuple2",_0: {ip: "Unknown"},_1: $Effects.none};
   var Model = function (a) {    return {ip: a};};
...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $App.init,view: $App.view,update: $App.update,inputs: _U.list([])});
   var main = app.html;
